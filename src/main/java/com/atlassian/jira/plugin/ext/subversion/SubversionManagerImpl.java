...
public class SubversionManagerImpl implements SubversionManager {
	...
	public synchronized void getLogEntries(long revision, final SvnEntryHandler svnEntryHandler) {
		...
		if (log.isDebugEnabled()) {
			log.debug(
					"Retrieved " + count.get() + " revisions to index (between " + retrieveStart + " and " + latestRevision + ") from repository=" + getRoot());
		}
	...
	public String getDisplayName() {
		return !properties.exists(MultipleSubversionRepositoryManager.SVN_REPOSITORY_NAME) ?
				getRoot() :
				properties.getString(MultipleSubversionRepositoryManager.SVN_REPOSITORY_NAME);
	}
	...
	public ViewLinkFormat getViewLinkFormat() {
		if (!isViewLinkSet) {
			String type = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_TYPE);
			String linkPathFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_PATH_KEY);
			String changesetFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_CHANGESET);
			String fileAddedFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_ADDED);
			String fileModifiedFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_MODIFIED);
			String fileReplacedFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_REPLACED);
			String fileDeletedFormat = properties.getString(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_DELETED);

			if (linkPathFormat != null || changesetFormat != null || fileAddedFormat != null || fileModifiedFormat != null || fileReplacedFormat != null
					|| fileDeletedFormat != null) {
				viewLinkFormat = new ViewLinkFormat(type, changesetFormat, fileAddedFormat, fileModifiedFormat, fileReplacedFormat, fileDeletedFormat,
						linkPathFormat);
			} else {
