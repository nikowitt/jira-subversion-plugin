...
public class SvnPropertiesLoader {
	...
	protected static SubversionProperties getSubversionProperty(int index, Properties props) {
		...
		if (props.containsKey(MultipleSubversionRepositoryManager.SVN_ROOT_KEY + indexStr)) {
			String svnRootStr = props.getProperty(MultipleSubversionRepositoryManager.SVN_ROOT_KEY + indexStr);
			String displayName = props.getProperty(MultipleSubversionRepositoryManager.SVN_REPOSITORY_NAME + indexStr);

			String changesetFormat = props.getProperty(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_CHANGESET + indexStr);
			String fileAddedFormat = props.getProperty(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_ADDED + indexStr);
			String fileModifiedFormat = props.getProperty(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_MODIFIED + indexStr);
			String fileReplacedFormat = props.getProperty(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_REPLACED + indexStr);
			String fileDeletedFormat = props.getProperty(MultipleSubversionRepositoryManager.SVN_LINKFORMAT_FILE_DELETED + indexStr);

			String username = props.getProperty(MultipleSubversionRepositoryManager.SVN_USERNAME_KEY + indexStr);
			String password = props.getProperty(MultipleSubversionRepositoryManager.SVN_PASSWORD_KEY + indexStr);
			String privateKeyFile = props.getProperty(MultipleSubversionRepositoryManager.SVN_PRIVATE_KEY_FILE + indexStr);
			Boolean revisionIndexing = null;
			if (props.containsKey(MultipleSubversionRepositoryManager.SVN_REVISION_INDEXING_KEY + indexStr)) {
				revisionIndexing = Boolean.valueOf(
						"true".equalsIgnoreCase(props.getProperty(MultipleSubversionRepositoryManager.SVN_REVISION_INDEXING_KEY + indexStr)));
			}
