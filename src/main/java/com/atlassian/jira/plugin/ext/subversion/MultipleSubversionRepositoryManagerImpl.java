...
public class MultipleSubversionRepositoryManagerImpl implements MultipleSubversionRepositoryManager, LifecycleAware, InitializingBean, DisposableBean {
	...
	private Map<Long, SubversionManager> loadManagersFromJiraProperties() {
		PropertySet propertySet = jiraPropertySetFactory.buildNoncachingPropertySet(APP_PROPERTY_PREFIX);
		long lastRepoId = propertySet.getLong(LAST_REPO_ID);

		// create the SubversionManagers
		Map<Long, SubversionManager> managers = Maps.newHashMap();
		for (long i = FIRST_REPO_ID; i <= lastRepoId; i++) {
			SubversionManager manager = createManagerFromPropertySet(i, jiraPropertySetFactory.buildCachingPropertySet(REPO_PROPERTY, i, true));
			if (manager != null) {
	...
	public SubversionManager createRepository(SvnProperties properties) {
		Lock lock = clusterLockService.getLockForName(REPOSITORY_INDEX_LOCK);
		PropertySet propertySet = jiraPropertySetFactory.buildNoncachingPropertySet(APP_PROPERTY_PREFIX);
		...
		SubversionManager subversionManager = createSubversionManager(properties, repositoryId);
	...
	protected SubversionManager createSubversionManager(SvnProperties properties, long repoId) {
		PropertySet set = jiraPropertySetFactory.buildCachingPropertySet(REPO_PROPERTY, repoId);
	...
	public void removeRepository(long repositoryId) {
		PropertySet original = jiraPropertySetFactory.buildCachingPropertySet(REPO_PROPERTY, repositoryId);
		...
		try {
			...
			throw new RuntimeException("Could not remove repository index", e);
		}
	}
